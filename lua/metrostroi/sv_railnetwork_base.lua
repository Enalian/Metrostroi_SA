...
hook.Add("OnEntityCreated","MetrostroiRailNetwork",function(ent)
	timer.Simple(0,function()
		if IsValid(ent) and ent.Base == "gmod_subway_base" then
			Metrostroi.SpawnedTrains[ent] = true
		end
	end)
end)
...
local function getFile(path,name,id)
	local typ = 0
	if file.Exists(Format(path..".txt",name),"DATA") then typ = bit.bor(typ,2) end
	if file.Exists(Format(path..".lua",name),"LUA") then typ = bit.bor(typ,1) end
	if typ == 0 then print(Format("%s definition file not found: %s",id,Format(path,name))) end
	return typ
end
function Metrostroi.Load(name,keep_signs)
	name = name or game.GetMap()
	local tracktype = getFile("metrostroi_data/track_%s",name,"Track")
	local signstype = getFile("metrostroi_data/signs_%s",name,"Signal")
	local autotype = getFile("metrostroi_data/auto_%s",name,"Autodrive")
	...
	if not keep_signs then
		...
		if bit.band(signstype,2) > 0 then
			signs= util.JSONToTable(file.Read(Format("metrostroi_data/signs_%s.txt",name)))
			print("Metrostroi: Loading signs definition...")
		end
		if not signs and bit.band(signstype,1) > 0 then
			signs= util.JSONToTable(file.Read(Format("metrostroi_data/signs_%s.lua",name),"LUA"))
		...
		if bit.band(autotype,2) > 0 then
			auto= util.JSONToTable(file.Read(Format("metrostroi_data/auto_%s.txt",name)))
			print("Metrostroi: Loading autodrive definition...")
		end
		if not auto and bit.band(autotype,1) > 0 then
			auto= util.JSONToTable(file.Read(Format("metrostroi_data/auto_%s.lua",name),"LUA"))
